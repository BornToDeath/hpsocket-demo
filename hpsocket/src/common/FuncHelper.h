...
template<typename T, typename V, typename E, typename = enable_if<is_same<decay<T>, decay<V>>::value && is_same<decay<V>, decay<E>>::value>>
...
template<typename T1, typename T2, typename = enable_if<is_same<decay<T1>, decay<T2>>::value>>
inline void CopyPlainObject(T1* p1, const T2* p2)
{
	CopyMemory(p1, p2, sizeof(T1));
}

template<typename T, typename C, typename = enable_if<is_integral<T>::value  && (is_same<C, char>::value || is_same<C, wchar_t>::value)>>
